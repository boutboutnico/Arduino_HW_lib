...
ft_void_t FT_GPU_Copro_SendCmd(FT_GPU_HAL_Context_t *phost, ft_const_uint32_t cmd) __attribute__ ( ( flatten ) );
ft_void_t FT_GPU_CoCmd_SendStr(FT_GPU_HAL_Context_t *phost, ft_const_char8_t *s) __attribute__ ( ( flatten ) );
ft_void_t FT_GPU_CoCmd_SendStr_P(FT_GPU_HAL_Context_t *phost, ft_prog_char8_t *s) __attribute__ ( ( flatten ) );
ft_void_t FT_GPU_CoCmd_StartFunc(FT_GPU_HAL_Context_t *phost, ft_const_uint16_t count) __attribute__ ( ( flatten ) );
ft_void_t FT_GPU_CoCmd_EndFunc(FT_GPU_HAL_Context_t *phost, ft_const_uint16_t count) __attribute__ ( ( flatten ) );

inline ft_void_t FT_GPU_Copro_SendCmd(FT_GPU_HAL_Context_t *phost, ft_const_uint32_t cmd)
{
	FT_GPU_HAL_TransferCmd(phost, cmd);
}

inline ft_void_t FT_GPU_CoCmd_SendStr(FT_GPU_HAL_Context_t *phost, ft_const_char8_t *s)
{
	FT_GPU_HAL_TransferString(phost, s);
}

inline ft_void_t FT_GPU_CoCmd_SendStr_P(FT_GPU_HAL_Context_t *phost, ft_prog_char8_t *s)
{
	FT_GPU_HAL_TransferString_P(phost, s);
}

inline ft_void_t FT_GPU_CoCmd_StartFunc(FT_GPU_HAL_Context_t *phost, ft_const_uint16_t count)
{
  FT_GPU_HAL_CheckCmdBuffer(phost, count);
  FT_GPU_HAL_StartCmdTransfer(phost, FT_GPU_WRITE );
}

inline ft_void_t FT_GPU_CoCmd_EndFunc(FT_GPU_HAL_Context_t *phost, ft_const_uint16_t count)
{
